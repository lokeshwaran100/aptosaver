      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ref={modalRef}
            className={cn(
              "min-h-[50%] max-h-[90%] md:max-w-[40%] bg-white dark:bg-neutral-950 border border-black dark:border-white md:rounded-2xl relative z-50 flex flex-col flex-1 overflow-hidden",
              className,
            )}
            initial={{
              opacity: 0,
...
export const ModalFooter = ({ children, className }: { children: ReactNode; className?: string }) => {
  return <div className={cn("flex justify-end p-4 bg-gray-400 dark:bg-neutral-900", className)}>{children}</div>;
};
